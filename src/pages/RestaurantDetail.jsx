...
const RestaurantDetail = () => {
	...
	useEffect(() => {
		const applyMockData = () => {
			...
			setActiveCategory(mockData.menu[0]?.title);
		...
		const fetchRestaurantDetails = async () => {
			...
			try {
				...
				// Swiggy's WAF returns 202 + empty body for this endpoint from a browser fetch
				if (!text) {
					console.warn("⚠️ Menu API blocked by Swiggy — using mock data");
					applyMockData();
					setLoading(false);
					return;
				}
				...
				console.warn(
					"⚠️ Live menu fetch failed, using mock data:",
					err.message,
				);
				applyMockData();
			} finally {
